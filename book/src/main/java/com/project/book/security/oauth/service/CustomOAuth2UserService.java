package com.project.book.security.oauth.service;
...
import com.project.book.security.oauth.OAuthAttributes;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		/**
		 * DefaultOAuth2UserService 객체를 생성하여, loadUser(userRequest)를 통해 DefaultOAuth2User 객체를 생성
		 * 소셜 로그인 API의 사용자 정보 제공 URI로 요청을 보내서 사용자 정보를 얻고,
		 * 이를 통해 DefaultOAuth2User 객체를 생성 후 반환
		 * 결과적으로 OAuth2User는 OAuth 서비스에서 가져온 유저 정보를 담고 있는 유저를 반환
		 */
		 ...
		 * '/oauth2/authorization/kakao' 에서 'kakao'가 registrationId
	...
	private UserEntity getUser(OAuthAttributes attributes, SocialType socialType) {
		// SocialType과 attributes에 들어있는 소셜 로그인의 식별값 id를 통해 회원을 찾음
